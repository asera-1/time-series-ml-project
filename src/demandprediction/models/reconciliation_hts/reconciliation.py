...
class To_Reconcile:
    ...
    def _save_model(self, file_path: str):
        save_dict = {
            ...
            "summing_mat": self.summing_mat,
            "lambd": self.lambd,
            "inputs_are_checked": self.inputs_are_checked
            }
    ...
    def _load_model(self, file_path: str):
        ...
        self.error_matrix = load_dict["error_matrix"]
        self.columns_ordered = load_dict["columns_ordered"]
        self.summing_mat = load_dict["summing_mat"]
        self.lambd = load_dict["lambd"]
    ...
    def reconcile(
        ...
        if method == 'OLS':
            ...
            if method == 'VS':
                ...
                W = W1
                print(f"W1 zero columns: {np.count_nonzero(np.diag(W1) == 0)}")
                print(f"W1 zero rows: {np.count_nonzero(np.diag(W1) == 0)}")
                condition_number = np.linalg.cond(W1)
                print("Condition number of W1:", condition_number)
                determinant = np.linalg.det(W1)
                print("Determinant of W1:", determinant)
            ...
            print(f"summing_matrix: {self.summing_mat}")
    ...
    def score(
        ...
        if metrics == 'rmse':
            score_dataframe = pd.DataFrame(data={'rmse': [root_mean_squared_error(
    ...
    def cross_score(
        ...
        if metrics == 'rmse':
            ...
            for index in tqdm(indexes):
                score_real.append(root_mean_squared_error(
                    self.real_values[:, index], self.base_forecasts[:, index],
                    ))
                score_reconciled.append(root_mean_squared_error(
            ...
            for index in tqdm(indexes):
                score_real.append(mean_absolute_error(
                    self.real_values[:, index], self.base_forecasts[:, index]))
                score_reconciled.append(mean_absolute_error(
                    self.real_values[:, index], self.reconcile(
                        method=reconcile_method, column_to_reconcile=index)))
        ...
        score_dataframe = pd.DataFrame(
            data={'base forecast': score_real,
                  f'reconciled forecast ({reconcile_method})': score_reconciled})
    ...
    def plot(
        ...
        if level == 'total':
            ...
            reconciled_forecasts = self.reconcile(
                    method=reconcile_method, reconcile_all=True
                )
            plt.plot(columns, reconciled_forecasts[0, columns],
                        color='green')
            if plot_real:
            ...
            idxes = indexes_of_series[level]
            if len(idxes) > 5:
                warnings.warn(
                    f"There are {len(indexes_of_series[level])} that are going"
                    " to be plotted"
                    " It is likely that the plot will be illisible"
                    "selecting a random subset of idxes for your protection"
                )
                idxes = random.sample(idxes, 5)

            fig, axes = plt.subplots(len(idxes), 1, figsize=(12, 2 * len(idxes)))

            for i, index in enumerate(idxes):
                axes[i].plot(columns, self.base_forecasts[index,
                         columns], color='red')
                if self.reconciled_forecasts is None:
                    self.reconciled_forecasts = self.reconcile(
                        method=reconcile_method)
                axes[i].plot(columns, self.reconciled_forecasts[index, columns],
                    color='green')
                if plot_real:
                    axes[i].plot(
                        columns,
        ...
        plt.savefig(f"{dir}/{level}_{reconcile_method}.png")
